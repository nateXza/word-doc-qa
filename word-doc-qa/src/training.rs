...
pub fn train_model() {
    ...
    let items: Vec<QAItem> = pairs.iter()
        .map(|p| {
            ...
            // Locate the answer token position in the encoded context.
            let ctx_tokens = vocab.encode(&p.context);
            let start_tok = ctx_start; // approximate: use context start as fallback
    ...
    O: Optimizer<QAModel<MyAutodiffBackend>, MyAutodiffBackend>,
{
    ...
    for batch in items.chunks(batch_size) {
        ...
        // Forward pass.
        let (start_logits, end_logits) = model.forward(input_ids);
...
fn save_checkpoint<B: Backend>(model: &QAModel<B>, epoch: usize, dir: &str) {
    // Burn's recorder API serialises model weights to a named file.
    use burn::record::{BinFileRecorder, FullPrecisionSettings, Recorder};

    let recorder = BinFileRecorder::<FullPrecisionSettings>::new();
    let path = format!("{}/model_epoch_{}", dir, epoch);

    match recorder.record(model.clone().into_record(), path.as_str().into()) {
